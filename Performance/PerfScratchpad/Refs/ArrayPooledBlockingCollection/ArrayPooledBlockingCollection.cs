...
namespace PerfScratchpad.Refs.ArrayPooledBlockingCollection
{
    ...
    /// <see cref="ArrayPooledBlockingCollection{T}"/> is used as a wrapper
    ...
    /// a <see cref="ArrayPooledBlockingCollection{T}"/> can be created to enforce
    ...
    /// <see cref="ArrayPooledBlockingCollection{T}"/> is similar to a traditional
    ...
    public class ArrayPooledBlockingCollection<T> : IEnumerable<T>, ICollection, IDisposable, IReadOnlyCollection<T>
    {
        // extensions
        private const int BufferPad = 32;
        
        private ArrayPool<T> _entries;
        //
        ...
        /// <summary>Gets the bounded capacity of this <see cref="ArrayPooledBlockingCollection{T}"/> instance.</summary>
        ...
        /// cref="ArrayPooledBlockingCollection{T}"/> has been disposed.</exception>
        ...
        /// <summary>Gets whether this <see cref="ArrayPooledBlockingCollection{T}"/> has been marked as complete for adding.</summary>
        ...
        /// <summary>Gets whether this <see cref="ArrayPooledBlockingCollection{T}"/> has been marked as complete for adding and is empty.</summary>
        ...
        /// <summary>Gets the number of items contained in the <see cref="ArrayPooledBlockingCollection{T}"/>.</summary>
        /// <value>The number of items contained in the <see cref="ArrayPooledBlockingCollection{T}"/>.</value>
        /// <exception cref="System.ObjectDisposedException">The <see
        /// cref="ArrayPooledBlockingCollection{T}"/> has been disposed.</exception>
        public int Count
        {
        ...
        /// <see cref="ArrayPooledBlockingCollection{T}"/>
        ...
        /// </remarks>
        public ArrayPooledBlockingCollection()
        ...
        /// The default underlying collection is a <see cref="System.Collections.Concurrent.ConcurrentQueue{T}">ConcurrentQueue&lt;T&gt;</see>.
        /// </remarks>
        public ArrayPooledBlockingCollection(IEventFactory<T> eventFactory, int bufferSize)
        {
            ...
            _entries = ArrayPool<T>.Create(bufferSize + 2 * BufferPad, 8);

            for (var i = 0; i < bufferSize; i++)
            {
                var buffer = _entries.Rent(BufferPad + i);

                buffer[i] = eventFactory.NewInstance();

                _entries.Return(buffer);
            }
        }

        /// <summary>Initializes a new instance of the <see
        /// cref="ArrayPooledBlockingCollection{T}"/>
        ...
        /// The default underlying collection is a <see cref="System.Collections.Concurrent.ConcurrentQueue{T}">ConcurrentQueue&lt;T&gt;</see>.
        /// </remarks>
        public ArrayPooledBlockingCollection(int boundedCapacity)
            : this(new ConcurrentQueue<T>(), boundedCapacity)
        {
        }

        /// <summary>Initializes a new instance of the <see cref="ArrayPooledBlockingCollection{T}"/>
        ...
        public ArrayPooledBlockingCollection(IProducerConsumerCollection<T> collection, int boundedCapacity)
        {
        ...
        /// <exception cref="System.ArgumentNullException">The <paramref name="collection"/> argument is
        /// null.</exception>
        public ArrayPooledBlockingCollection(IProducerConsumerCollection<T> collection)
        {
            ArgumentNullException.ThrowIfNull(collection);
        ...
        /// <summary>
        /// Adds the item to the <see cref="ArrayPooledBlockingCollection{T}"/>.
        /// </summary>
        /// <param name="item">The item to be added to the collection. The value can be a null reference.</param>
        /// <exception cref="System.InvalidOperationException">The <see
        /// cref="ArrayPooledBlockingCollection{T}"/> has been marked
        ...
        /// <see cref="ArrayPooledBlockingCollection{T}"/> was initialized,
        ...
        /// <summary>
        /// Attempts to add the specified item to the <see cref="ArrayPooledBlockingCollection{T}"/>.
        /// </summary>
        ...
        private bool TryAddWithNoTimeValidation(T item, int millisecondsTimeout, CancellationToken cancellationToken)
        {
            ...
            if (waitForSemaphoreWasSuccessful)
            {
                ...
                while (true)
                {
                    ...
                    if (Interlocked.CompareExchange(ref _currentAdders, observedAdders + 1, observedAdders) ==
                        observedAdders)
                    {
                        Debug.Assert((observedAdders + 1) <= (~COMPLETE_ADDING_ON_MASK),
        ...
        /// <summary>Takes an item from the <see cref="ArrayPooledBlockingCollection{T}"/>.</summary>
        /// <returns>The item removed from the collection.</returns>
        /// <exception cref="System.ObjectDisposedException">The <see
        /// cref="ArrayPooledBlockingCollection{T}"/> has been disposed.</exception>
        /// <exception cref="System.InvalidOperationException">The underlying collection was modified
        /// outside of this <see
        /// cref="ArrayPooledBlockingCollection{T}"/> instance, or the <see
        /// cref="ArrayPooledBlockingCollection{T}"/> is empty and has been marked
        /// as complete with regards to additions.</exception>
        ...
        /// <summary>
        /// Attempts to remove an item from the <see cref="ArrayPooledBlockingCollection{T}"/>.
        /// </summary>
        ...
        /// <exception cref="System.InvalidOperationException">The underlying collection was modified
        /// outside of this <see
        /// cref="ArrayPooledBlockingCollection{T}"/> instance.</exception>
        ...
        /// <exception cref="OperationCanceledException">If the <see cref="CancellationToken"/> is canceled.</exception>
        /// <exception cref="System.ObjectDisposedException">If the collection has been disposed.</exception>
        private bool TryTakeWithNoTimeValidation([MaybeNullWhen(false)] out T item, int millisecondsTimeout,
            CancellationToken cancellationToken, CancellationTokenSource? combinedTokenSource)
        {
            CheckDisposed();
            item = default(T)!;
        ...
        /// <see cref="ArrayPooledBlockingCollection{T}"/> instances.
        /// </summary>
        ...
        /// cref="ArrayPooledBlockingCollection{T}"/> instances has been disposed.</exception>
        ...
        /// <see cref="ArrayPooledBlockingCollection{T}"/> instances were initialized,
        /// a call to AddToAny may block until space is available in one of the collections
        /// to store the provided item.
        /// </remarks>
        public static int AddToAny(ArrayPooledBlockingCollection<T>[] collections, T item)
        {
#if DEBUG
        ...
        /// to store the provided item.
        /// </remarks>
        public static int AddToAny(ArrayPooledBlockingCollection<T>[] collections, T item, CancellationToken cancellationToken)
        {
#if DEBUG
            int tryAddAnyReturnValue =
#else
        ...
        /// <exception cref="System.ArgumentOutOfRangeException">The count of <paramref name="collections"/> is greater than the maximum size of
        /// 62 for STA and 63 for MTA.</exception>
        public static int TryAddToAny(ArrayPooledBlockingCollection<T>[] collections, T item)
        {
        ...
        /// <exception cref="System.ArgumentOutOfRangeException">The count of <paramref name="collections"/> is greater than the maximum size of
        /// 62 for STA and 63 for MTA.</exception>
        public static int TryAddToAny(ArrayPooledBlockingCollection<T>[] collections, T item, TimeSpan timeout)
        {
            ValidateTimeout(timeout);
        ...
        /// <exception cref="System.ArgumentOutOfRangeException">The count of <paramref name="collections"/> is greater than the maximum size of
        /// 62 for STA and 63 for MTA.</exception>
        public static int TryAddToAny(ArrayPooledBlockingCollection<T>[] collections, T item, int millisecondsTimeout)
        {
            ValidateMillisecondsTimeout(millisecondsTimeout);
        ...
        private static int TryAddToAnyCore(ArrayPooledBlockingCollection<T>[] collections, T item, int millisecondsTimeout,
            CancellationToken externalCancellationToken)
        {
            ...
            List<WaitHandle> handles = GetHandles(collections, externalCancellationToken, true,
            ...
            while (millisecondsTimeout == Timeout.Infinite || timeout >= 0)
            {
                       ...
                       CancellationTokenSource.CreateLinkedTokenSource(collatedCancellationTokens))
                {
                    ...
                    if (linkedTokenSource.IsCancellationRequested)
                    {
                        ...
                        throw new ArgumentException(SR.BlockingCollection_CantAddAnyWhenCompleted,
        ...
        private static int TryAddToAnyFast(ArrayPooledBlockingCollection<T>[] collections, T item)
        {
        ...
        private static List<WaitHandle> GetHandles(ArrayPooledBlockingCollection<T>[] collections,
            CancellationToken externalCancellationToken, bool isAddOperation,
            out CancellationToken[] cancellationTokens)
        {
            Debug.Assert(collections != null);
            List<WaitHandle>
                handlesList =
                    new List<WaitHandle>(collections.Length + 1); // + 1 for the external token handle to be added
            List<CancellationToken>
                tokensList = new List<CancellationToken>(collections.Length + 1); // + 1 for the external token
            ...
            if (isAddOperation)
            {
                for (int i = 0; i < collections.Length; i++)
                {
                    ArrayPooledBlockingCollection<T> c = collections[i];
        ...
        public static int TakeFromAny(ArrayPooledBlockingCollection<T>[] collections, out T? item)
        {
        ...
        public static int TryTakeFromAny(ArrayPooledBlockingCollection<T>[] collections, out T? item)
        {
        ...
        /// 62 for STA and 63 for MTA.</exception>
        /// <remarks>A call to TryTakeFromAny may block until an item is available to be removed.</remarks>
        public static int TryTakeFromAny(ArrayPooledBlockingCollection<T>[] collections, out T? item, TimeSpan timeout)
        {
            ValidateTimeout(timeout);
        ...
        /// 62 for STA and 63 for MTA.</exception>
        /// <remarks>A call to TryTakeFromAny may block until an item is available to be removed.</remarks>
        public static int TryTakeFromAny(ArrayPooledBlockingCollection<T>[] collections, out T? item, int millisecondsTimeout)
        {
            ValidateMillisecondsTimeout(millisecondsTimeout);
        ...
        /// null element. Also, if at least one of the collections has been marked complete for adds.</exception>
        /// <exception cref="System.ObjectDisposedException">If at least one of the collections has been disposed.</exception>
        private static int TryTakeFromAnyCore(ArrayPooledBlockingCollection<T>[] collections, out T? item,
            int millisecondsTimeout, bool isTakeOperation, CancellationToken externalCancellationToken)
        {
            ...
            for (int i = 0; i < collections.Length; i++)
            {
                ...
                if (!collections[i].IsCompleted && collections[i]._occupiedNodes.CurrentCount > 0 &&
                    collections[i].TryTake(out item))
                    return i;
            }

            //Fast path failed, try the slow path
            return TryTakeFromAnyCoreSlow(collections, out item, millisecondsTimeout, isTakeOperation,
        ...
        /// null element. Also, if at least one of the collections has been marked complete for adds.</exception>
        /// <exception cref="System.ObjectDisposedException">If at least one of the collections has been disposed.</exception>
        private static int TryTakeFromAnyCoreSlow(ArrayPooledBlockingCollection<T>[] collections, out T? item,
            int millisecondsTimeout, bool isTakeOperation, CancellationToken externalCancellationToken)
        {
            const int OPERATION_FAILED = -1;
            ...
            while (millisecondsTimeout == Timeout.Infinite || timeout >= 0)
            {
                ...
                CancellationToken[] collatedCancellationTokens;
                List<WaitHandle> handles = GetHandles(collections, externalCancellationToken, false,
                    out collatedCancellationTokens);
                       ...
                       CancellationTokenSource.CreateLinkedTokenSource(collatedCancellationTokens))
                {
                    ...
                    if (!linkedTokenSource
                            .IsCancellationRequested) // if neither internal nor external cancellation requested
                    {
                        Debug.Assert((index == WaitHandle.WaitTimeout) || (index >= 0 && index < handles.Count));
                        if (index == WaitHandle.WaitTimeout) //case#2
                            break;
        ...
        /// <summary>
        /// Marks the <see cref="ArrayPooledBlockingCollection{T}"/> instances
        ...
        public void CompleteAdding()
        {
            ...
            while (true)
            {
                ...
                if (Interlocked.CompareExchange(ref _currentAdders, observedAdders | COMPLETE_ADDING_ON_MASK,
        ...
        /// <summary>
        /// Releases resources used by the <see cref="ArrayPooledBlockingCollection{T}"/> instance.
        /// </summary>
        public void Dispose()
        {
            Dispose(true);
        ...
        /// <summary>Copies the items from the <see cref="ArrayPooledBlockingCollection{T}"/> instance into a new array.</summary>
        ...
        /// <summary>Copies all of the items in the <see cref="ArrayPooledBlockingCollection{T}"/> instance
        /// to a compatible one-dimensional array, starting at the specified index of the target array.
        /// </summary>
        /// <param name="array">The one-dimensional array that is the destination of the elements copied from
        /// the <see cref="ArrayPooledBlockingCollection{T}"/> instance. The array must have zero-based indexing.</param>
        ...
        /// null element. Also, if at least one of the collections has been marked complete for adds.</exception>
        /// <exception cref="System.ObjectDisposedException">If at least one of the collections has been disposed.</exception>
        private static void ValidateCollectionsArray(ArrayPooledBlockingCollection<T>[] collections, bool isAddOperation)
        {
        ...
        public void PublishEvent(int @event)
        {
            ...
            var buffer = _entries.Rent(dataSize) as IntEvent;
    ...
    {
        private readonly ArrayPooledBlockingCollection<T> _arrayPooledBlockingCollection; // The collection being viewed.
        ...
        public BlockingCollectionDebugView(ArrayPooledBlockingCollection<T> collection)
        {
            ArgumentNullException.ThrowIfNull(collection);

            _arrayPooledBlockingCollection = collection;
        }
        ...
        public T[] Items
        {
            get { return _arrayPooledBlockingCollection.ToArray(); }
        }
    }
}